...
object ConsoleInputParser:
  ...
  def compute(input: String): ConsoleInput =
    ...
    val tickOpt = doubleR.findFirstIn(input).map(parse[Double](_))
    val depthOpt = tokens.lastOption.map(parse[Int](_))

    println(s"compute : $fileNameOpt, $tickOpt, $depthOpt")
  ...
  given ParseOp[Double] = ParseOp[Double](_.toDouble)
  given ParseOp[Int] = ParseOp[Int](_.toInt)
