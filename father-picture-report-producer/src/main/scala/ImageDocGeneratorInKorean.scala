...
object ImageDocGeneratorInKorean extends SimpleSwingApplication:
  ...
  trait ReactiveControls extends MainFrame:
    ...
    val folderPathLabel = new Label("이미지가 있는 폴더 선택:")
    val folderPathTextField = new TextField(20)
    val browseButton = new Button("찾아보기")
    val titleLabel = new Label("문서 제목:")
    val titleTextField = new TextField(20)
    val outputFileLabel = new Label("출력 파일 이름:")
    val outputFileTextField = new TextField("output.docx")
    val generateButton = new Button("문서 생성")
      ...
      case ButtonClicked(`generateButton`) =>
        ...
        if folderPath.nonEmpty && docTitle.nonEmpty && initialOutputFile.nonEmpty then
          getUniqueFileName(initialOutputFile) match
            ...
            case None =>
              Dialog.showMessage(contents.head, "문서 생성이 취소되었습니다.", title = "취소됨")
        else
          Dialog.showMessage(contents.head, "모든 필드를 채워주세요.", title = "오류")
    }
    ...
    final def getUniqueFileName(fileName: String): Option[String] =
      if !new File(fileName).exists() then
        ...
        Dialog.showInput(null, "파일이 이미 존재합니다. 새 파일 이름을 입력하세요:", initial = fileName) match
    ...
    def showProgressDialog(folderPath: String, outputFile: String, docTitle: String): Unit =
      val progressDialog = new Dialog:
        ...
        contents = new BoxPanel(Orientation.Vertical):
          contents += new Label("문서를 생성하는 중입니다. 잠시만 기다려주세요...")
          contents += Swing.VStrut(10)
      ...
      val worker = new SwingWorker[Unit, Unit]:
        ...
        override def done(): Unit =
          ...
          Dialog.showMessage(contents.head, "문서가 성공적으로 생성되었습니다!", title = "성공")
          enableControls()
  ...
  def calculateScaledDimensions(width: Int, height: Int, maxWidth: Int, maxHeight: Int): (Int, Int) =
  ...
  def convertBufferedImageToInputStream(image: BufferedImage): ByteArrayInputStream = {
